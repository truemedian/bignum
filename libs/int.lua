...
--- Performs comparison on the limbs `a[a_n]...a[a_i]` by `b[b_n]...b[b_i]` and returns -1 if |a| < |b|, 0 if |a| == |b|, 1 if |a| > |b|.
local function limbwiseCompareAbs(a, a_i, a_n, b, b_i, b_n)
	if a_n ~= b_n then
		return a_n < b_n and -1 or 1
	end

	for i = a_n - 1, 0, -1 do
		if a[a_i + i] ~= b[b_i + i] then
			return a[a_i + i] < b[b_i + i] and -1 or 1
		end
	end
...
--- Performs addition on the limbs `a[a_n]...a[a_i]` by `b[b_n]...b[b_i]` and stores the result in `r[r_n]...r[r_i]`. Returns the carry bit.
local function limbwiseAddWithCarry(r, r_i, a, a_i, a_n, b, b_i, b_n)
	assert(a_n >= b_n)
	...
	for i = 0, b_n - 1 do
		ov1, c1 = addWithOverflow(a[a_i + i], b[b_i + i])
		r[r_i + i], c2 = addWithOverflow(ov1, carry)
		carry = c1 + c2
	end

	-- perform the carry on the remaining limbs
	for i = b_n, a_n - 1 do
		r[r_i + i], carry = addWithOverflow(a[a_i + i], carry)
	end
...
--- Performs subtraction on the limbs `a[a_n]...a[a_i]` by `b[b_n]...b[b_i]` and stores the result in `r[r_n]...r[r_i]`. Returns the borrow bit.
local function limbwiseSubtractWithBorrow(r, r_i, a, a_i, a_n, b, b_i, b_n)
	assert(a_n >= b_n)
	...
	for i = 0, b_n - 1 do
		local ov1, c1 = subWithOverflow(a[a_i + i], b[b_i + i])
	...
	for i = b_n, a_n - 1 do
		r[r_i + i], borrow = subWithOverflow(a[a_i + i], borrow)
	end

	return borrow
end

--- Performs multiplication on the limbs `a[a_n]...a[1]` by a scalar `b_x` and stores the result in `r[r_n]...r[r_i]`.
local function limbwiseAddMulScalar(r, r_i, a, a_i, a_n, b_x)
	if b_x == 0 then
		return
	end
	...
	local carry = 0
	for j = 0, a_n - 1 do
		r[r_i + j], carry = addMulWithCarry(r[r_i + j], a[a_i + j], b_x, carry)
	end

	while carry ~= 0 do
		r[r_i + a_n], carry = addWithOverflow(r[r_i + a_n], carry)

		r_i = r_i + 1
	end
end

local limbwiseMultiply -- pre-definition for karatsuba

--- Performs karatsuba multiplication on the limbs `a[a_n]...a[1]` by `b[b_n]...b[1]` and stores the result in `r[r_n]...r[r_i]`.
local function limbwiseKaratsuba(r, r_i, a, a_i, a_n, b, b_i, b_n)
	-- a = a1 * B + a0, a0 < B
	-- b = b1 * B + b0, b0 < B
	...
	local a0, a0_i, a0_n = a, a_i, split
	local a1, a1_i, a1_n = a, a_i + split, a_n - split

	local b0, b0_i, b0_n = b, b_i, split
	local b1, b1_i, b1_n = b, b_i + split, b_n - split
	...
	limbwiseMultiply(p2, 1, a1, a1_i, math.min(a1_n, p2_limbs), b1, b1_i, math.min(b1_n, p2_limbs))
	p2_n = limbwiseNormalize(p2, a1_n + b1_n)

	-- add p2 * B
	limbwiseAddWithCarry(r, r_i + split, r, r_i + split, r_n - split, p2, 1, p2_n)

	-- add p2 * B^2
	if limbs_after_split2 > 0 then
		limbwiseAddWithCarry(r, r_i + split * 2, r, r_i + split * 2, r_n - split * 2, p2, 1, math.min(p2_n, limbs_after_split2))
	end

	-- compute p0
	local p0_limbs = math.min(limbs_after_split, a0_n + b0_n)
	local p0, p0_n = {}, 0
	for i = 1, p0_limbs do
		p0[i] = 0
	end

	limbwiseMultiply(p0, 1, a0, a0_i, a0_n, b0, b0_i, b0_n)
	p0_n = limbwiseNormalize(p0, a0_n + b0_n)

	-- add p0
	limbwiseAddWithCarry(r, r_i, r, r_i, r_n, p0, 1, p0_n)

	-- add p0 * B
	limbwiseAddWithCarry(r, r_i + split, r, r_i + split, r_n - split, p0, 1, math.min(p0_n, limbs_after_split))

	-- compute p1
	local a0x, a0x_i, a0x_n = a, a_i, math.min(a0_n, limbs_after_split)
	local b0x, b0x_i, b0x_n = b, b_i, math.min(b0_n, limbs_after_split)

	local j0_sign = limbwiseCompareAbs(a0x, a0x_i, a0x_n, a1, a1_i, a1_n)
	local j1_sign = limbwiseCompareAbs(b1, b1_i, b1_n, b0x, b0x_i, b0x_n)
	...
	if j0_sign == 1 then
		-- a0 > a1
		limbwiseSubtractWithBorrow(j0, 1, a0x, a0x_i, a0x_n, a1, a1_i, a1_n)
	else
		-- a0 < a1
		limbwiseSubtractWithBorrow(j0, 1, a1, a1_i, a1_n, a0x, a0x_i, a0x_n)
	end
	j0_n = limbwiseNormalize(j0, limbs_after_split)

	if j1_sign == 1 then
		-- b1 > b0
		limbwiseSubtractWithBorrow(j1, 1, b1, b1_i, b1_n, b0x, b0x_i, b0x_n)
	else
		-- b1 < b0
		limbwiseSubtractWithBorrow(j1, 1, b0x, b0x_i, b0x_n, b1, b1_i, b1_n)
	end
	j1_n = limbwiseNormalize(j1, limbs_after_split)

	if j0_sign == j1_sign then
		-- both positive or both negative
		limbwiseMultiply(r, r_i + split, j0, 1, j0_n, j1, 1, j1_n)
	else
		-- one is positive, one is negative, we need a temporary because limbwiseMultiply cannot subtract
		local p1, p1_n = {}, 0
		for i = 1, limbs_after_split do
			p1[i] = 0
		end

		limbwiseMultiply(p1, 1, j1, 1, j1_n, j0, 1, j0_n)
		p1_n = limbwiseNormalize(p1, limbs_after_split)

		-- subtract p1
		limbwiseSubtractWithBorrow(r, r_i + split, r, r_i + split, r_n - split, p1, 1, p1_n)
	end
end

--- Performs multiplication the limbs `a[a_n]...a[1]` by `b[b_n]...b[1]` and stores the result in `r[r_n]...r[r_i]`.
function limbwiseMultiply(r, r_i, a, a_i, a_n, b, b_i, b_n)
	if a_n < b_n then
		a, b = b, a
		a_n, b_n = b_n, a_n
	end
	...
	if b_n > 96 then
		return limbwiseKaratsuba(r, r_i, a, a_i, a_n, b, b_i, b_n)
	end

	-- long multiplication
	for i = 0, b_n - 1 do
		limbwiseAddMulScalar(r, r_i + i, a, a_i, a_n, b[b_i + i])
	end
end
...
function Integer.orderAbs(a, b)
	return limbwiseCompareAbs(a.limbs, 1, a.n, b.limbs, 1, b.n)
end
...
function Integer.add(r, a, b)
	...
	elseif a.neg ~= b.neg then
		...
		carry = limbwiseSubtractWithBorrow(r.limbs, 1, a.limbs, 1, a.n, b.limbs, 1, b.n)
		...
		carry = limbwiseAddWithCarry(r.limbs, 1, a.limbs, 1, a.n, b.limbs, 1, b.n)
		r.n = limbwiseNormalize(r.limbs, a.n)
		r.neg = a.neg
	end
...
function Integer.mulNoAliasScalar(r, a, b)
	...
	elseif c < RADIX then
		...
		limbwiseAddMulScalar(r.limbs, 1, a.limbs, 1, a.n, c)
...
function Integer.mulNoAlias(r, a, b)
	...
	elseif b.n == 1 then
		...
		limbwiseMultiply(r.limbs, 1, a.limbs, 1, a.n, b.limbs, 1, b.n)
...
function Integer.divTrunc(q, r, a, b)
	...
	assert(not rawequal(q.limbs, r.limbs), "q must not alias r")
