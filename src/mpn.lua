...
local LIMB_NUMBER_PRECISION = math.ceil(64 / LIMB_SIZE)
...
function mpn.add_1(r, r0, a, a0, n, y)
	...
	-- if we get here, then the last addition caused a carry
	__validate_dest_suffix(r, r0, n)
	return 1
end
...
function mpn.submul_1(r, r0, a, a0, n, y)
	...
	for i = 1, n do
		...
		borrow = rk < 0 and math.ceil(-rk / LIMB_RADIX) or 0
		r[r0 + i] = band(rk, LIMB_MAX)
	end
...
--- Computes `r[r0:2*n] = a[a0:n] * b[b0:n]`. Returns the most significant limb.
---
...
--- Computes `r[r0:an+bn] = a[a0:an] * b[b0:bn]`. Returns the most significant limb.
---
--- Requires `r` must not alias `a` or `b`.
---@param r mpn.limbs
---@param r0 mpn.offset
...
function mpn.mul(r, r0, a, a0, an, b, b0, bn)
	...
	-- follows where the carry from an individual multiplication should go
	local t = r0 + an + 1

	-- do the first multiplication, this saves a loop to zero `r`.
	r[t] = mpn.mul_1(r, r0, a, a0, an, b[b0 + 1])
...
--- Computes `r[r0:n] = a[a0:n] << tcnt`. Returns the carried limb.
---@param r mpn.limbs
---@param r0 mpn.offset
---@param a mpn.limbs_const
---@param a0 mpn.offset
...
function mpn.lshift(r, r0, a, a0, n, tcnt)
	...
	local mlmb = math.floor(tcnt / LIMB_SIZE)
	local cnt = tcnt - mlmb * LIMB_SIZE
	local tnc = LIMB_SIZE - cnt

	local overflow = rshift(a[a0 + n], tnc)
	local low_half = band(lshift(a[a0 + n], cnt), LIMB_MAX)
	for i = n - 1, 1, -1 do
		local t = a[a0 + i]
		local high_half = rshift(t, tnc)
		r[r0 + i + mlmb + 1] = bor(high_half, low_half)
		low_half = band(lshift(t, cnt), LIMB_MAX)
	end

	r[r0 + mlmb + 1] = low_half
...
--- Computes `r[r0:n] = a[a0:n] >> tcnt`. Returns the overflowed limb.
---@param r mpn.limbs
---@param r0 mpn.offset
---@param a mpn.limbs_const
---@param a0 mpn.offset
...
function mpn.rshift(r, r0, a, a0, n, tcnt)
	...
	local mlmb = math.floor(tcnt / LIMB_SIZE)
	local cnt = tcnt % LIMB_SIZE
	local tnc = LIMB_SIZE - cnt

	local overflow = rshift(a[a0 + 1], cnt)
	local high_half = rshift(a[a0 + 1], cnt)
	for i = 1, n - mlmb - 1 do
		local t = a[a0 + i + mlmb + 1]
		local low_half = band(lshift(t, tnc), LIMB_MAX)
		r[r0 + i] = bor(low_half, high_half)
		high_half = rshift(t, cnt)
	end

	-- fill the remaining limbs with zeroes if we did a limb shift
	if tcnt >= LIMB_SIZE then
		r[r0 + n - mlmb] = high_half
		for i = n - mlmb + 1, n do
			r[r0 + i] = 0
		end
...
function mpn.divmod(q, q0, r, r0, n, n0, nn, d, d0, dn)
	...
	assert(mpn.lshift(d, d0, d, d0, dn, norm_shift) == 0) -- cannot overflow, we shift only to the highest bit
	local noverfow = mpn.lshift(n, n0, n, n0, nn, norm_shift)
	if noverfow ~= 0 then
		n[n0 + nn + 1] = noverfow
		nn = nn + 1
	end
	...
	-- 2. while n >= d * b^(nn - dn)
	while mpn.cmp_n(n, n0 + nn - dn, d, d0, dn) >= 0 do
		-- 2.1 set q[nn - dn] += 1
	...
	for i = nn, dn + 1, -1 do
		...
		-- 3.1 if n[i] == d[dn]
		if n[n0 + i] == d[d0 + dn] then
			-- 3.1.1 then set q[k] = b - 1
			q[q0 + k] = LIMB_MAX
		else
			-- 3.1.2 otherwise set q[k] = floor((n[i] * b + n[i - 1]) / d[dn])
			q[q0 + k] = math.floor((n[n0 + i] * LIMB_RADIX + n[n0 + i - 1]) / d[d0 + dn])
		end

		local y = {}
		local x = { n[n0 + i - 2] or 0, n[n0 + i - 1] or 0, n[n0 + i] }

		-- 3.2 while q[k] * (d[dn] * b + d[dn - 1]) > (n[i] * b^2 + n[i - 1] * b + n[i - 2])
		while true do
			y[1] = d[d0 + dn - 1] or 0
			y[2] = d[d0 + dn]
			y[3] = mpn.mul_1(y, 0, y, 0, 2, q[q0 + k])

			if mpn.cmp_n(y, 0, x, 0, 3) > 0 then
				-- 3.2.1 set q[k] -= 1
				q[q0 + k] = q[q0 + k] - 1
			else
				break
		...
		local overflow = mpn.submul_1(n, n0 + k - 1, d, d0, dn, q[q0 + k])
		overflow = mpn.sub_1(n, n0 + i - 1, n, n0 + i - 1, nn - i + 1, overflow)

		-- 3.4 if n < 0 then
		if overflow > 0 then
			-- 3.4.1 set x += d * b^(i - dn - 1)
			mpn.add_n(n, n0 + i - dn, n, n0 + i - dn, d, d0, dn)
...
function mpn.divmod_1(q, q0, n, n0, nn, z)
	...
	for i = nn, 1, -1 do
		...
		q[q0 + i] = band(math.floor(partial / z), LIMB_MAX)
		remainder = math.floor(partial % z)
	end
...
function mpn.sqrtrem(r, r0, e, e0, s, s0, n)
	__validate_dest(r, r0, math.ceil(n / 2))
	...
	while true do
		...
		if x1n - x0n <= 1 then
			...
			if mpn.cmp(check, 0, x1n, x0, 0, x0n) == 0 then
				...
				mpn.sqr(t, 0, r, r0, x0n)
				tn = mpn.normalized_size(t, 0, 2 * x0n)
...
function mpn.is_perfect_square(a, a0, n)
	...
	local first_limb = a[a0 + 1]
	if band(first_limb, 1) ~= 0 then
		return false -- odd numbers cannot be perfect squares
	end

	-- check if the number of bits set is even
	return mpn.popcount(a, a0, n) % 2 == 0
end
...
--- Returns true if the bit at `idx` in `a[a0:n]` is set.
---@param a mpn.limbs_const
---@param a0 mpn.offset
---@param n mpn.size
---@param idx mpn.size
---@return boolean
function mpn.btest(a, a0, n, idx)
	__validate_source(a, a0, n)

	local limb = rshift(idx, LIMB_SIZE) + 1
	local offset = band(idx, LIMB_MAX)

	return band(a[a0 + limb], lshift(1, offset)) ~= 0
end
...
function mpn.bextract(r, r0, a, a0, n, idx, width)
	...
	local limb_width_max = math.ceil((width + inner_offset) / LIMB_SIZE)
	local limb_width = math.ceil(width / LIMB_SIZE)
...
---@param b0 mpn.offset
---@param n mpn.size
function mpn.bandn_n(r, r0, a, a0, b, b0, n)
	__validate_dest(r, r0, n)
	__validate_source(a, a0, n)
	__validate_source(b, b0, n)

	for i = 1, n do
		r[r0 + i] = band(a[a0 + i], bnot(b[b0 + i]))
	end
...
---@param b0 mpn.offset
---@param n mpn.size
function mpn.biorn_n(r, r0, a, a0, b, b0, n)
	__validate_dest(r, r0, n)
	__validate_source(a, a0, n)
	__validate_source(b, b0, n)

	for i = 1, n do
		r[r0 + i] = band(bor(a[a0 + i], bnot(b[b0 + i])), LIMB_MAX)
	end
...
---@param b0 mpn.offset
---@param n mpn.size
function mpn.bnand_n(r, r0, a, a0, b, b0, n)
	__validate_dest(r, r0, n)
	__validate_source(a, a0, n)
	__validate_source(b, b0, n)

	for i = 1, n do
		r[r0 + i] = band(bnot(band(a[a0 + i], b[b0 + i])), LIMB_MAX)
	end
...
---@param b0 mpn.offset
---@param n mpn.size
function mpn.binor_n(r, r0, a, a0, b, b0, n)
	__validate_dest(r, r0, n)
	__validate_source(a, a0, n)
	__validate_source(b, b0, n)

	for i = 1, n do
		r[r0 + i] = band(bnot(bor(a[a0 + i], b[b0 + i])), LIMB_MAX)
	end
...
---@param b0 mpn.offset
---@param n mpn.size
function mpn.bxnor_n(r, r0, a, a0, b, b0, n)
	__validate_dest(r, r0, n)
	__validate_source(a, a0, n)
	__validate_source(b, b0, n)

	for i = 1, n do
		r[r0 + i] = band(bnot(bxor(a[a0 + i], b[b0 + i])), LIMB_MAX)
	end

	__validate_dest_suffix(r, r0, n)
end

--- Computes `r[r0:n] = ~a[a0:n]`
---@param r mpn.limbs
---@param r0 mpn.offset
---@param a mpn.limbs_const
---@param a0 mpn.offset
---@param n mpn.size
function mpn.bnot_n(r, r0, a, a0, n)
	__validate_dest(r, r0, n)
	__validate_source(a, a0, n)

	for i = 1, n do
		r[r0 + i] = band(bnot(a[a0 + i]), LIMB_MAX)
	end
